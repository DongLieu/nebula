...
Example:
	nebulad testnet --v 4 --output-dir ./output --starting-ip-address 192.168.10.2
	`,
...
func InitTestnet(
	...
	if chainID == "" {
		chainID = "nebula-" + tmrand.NewRand().Str(6)
	}
	...
	for i := 0; i < numValidators; i++ {
		...
		coins := sdk.Coins{
			sdk.NewCoin(app.DefaultDenom, sdk.NewInt(1000000000)),
			sdk.NewCoin("uusdt", sdk.NewInt(1000000000)),
		}
...
func initGenFiles(
	...
	stakingGenState.Params.BondDenom = app.DefaultDenom
	appGenState[stakingtypes.ModuleName] = clientCtx.Codec.MustMarshalJSON(&stakingGenState)

	var govGenState govtypes.GenesisState
	clientCtx.Codec.MustUnmarshalJSON(appGenState[govtypes.ModuleName], &govGenState)

	govGenState.DepositParams.MinDeposit[0].Denom = app.DefaultDenom
	appGenState[govtypes.ModuleName] = clientCtx.Codec.MustMarshalJSON(&govGenState)

	var crisisGenState crisistypes.GenesisState
	clientCtx.Codec.MustUnmarshalJSON(appGenState[crisistypes.ModuleName], &crisisGenState)

	crisisGenState.ConstantFee.Denom = app.DefaultDenom
